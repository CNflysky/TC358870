...
void RS3() {
  ...
  i2c1_uh2cd_write32(0x0370, 0x00000411);  // APF_VDELAYCNT
...
void RS_Int() {
  ...
  switch (stage) {
    case 1:
      printf("Current stage: State S1 --- Stand By\n");
      break;
    case 2:
      printf("Current stage: State S2 --- Ready\n");
      break;
    case 3:
      printf("Current stage: State S3 --- Video Transmission\n");
      break;
    default:
      printf("Current stage: State S? --- Unknown: stage code = %d\n", stage);
      break;
  }
  ...
  if (int_status & 0x20) {
    ...
    if (int_sys_status & 0x01) {
      RS2();
      stage = 2;
      printf("Executed RS2 --- Stage S2: Ready\n");
    }
  }
  // check bit 9
  if (int_status & 0x0200) {
    ...
    if (misc_int & 0x02 && sys_status & 0x80) {
      turn_on_panel();
      // turn on audio
      board_audio_control(GPIO_PIN_SET);
      stage = 3;
      printf("Executed RS3 --- Stage S3: Video Transmission\n");
    }
  }
  ...
  if (misc_int & 0x02 && sys_status & 0x01 && !(sys_status & 0x80)) {
    ...
    stage = 2;
    printf("Executed RS4 --- Stage S2: Ready\n");
  }
  ...
  if (sys_int & 0x01 && !(sys_status & 0x01)) {
    ...
    board_audio_control(GPIO_PIN_RESET);
    stage = 1;
    printf("Executed RS5 --- Stage S1: Stand by\n");
    // check bit 0 and bit 0
